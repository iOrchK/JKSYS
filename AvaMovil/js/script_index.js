$(document).ready(function(){
	...
	$("#btn-avaluar").click(function(){
		var url="get_valuacion.php";
		accionBoton(url);
	});

	$("#btn-consulta-libre").click(function(){
		var url="get_consulta_libre.php";
		accionBoton(url);
	});
...
function accionBoton(url){
		...
		if(dispositivo === "" || accesorio === "" || estado === ""){
			...
			if(isNaN(iddispositivo) || isNaN(idestado) || isNaN(idaccesorios)){
				...
				$.ajax({
					type: "get",
					url: "php/"+url+"?id_movil="+iddispositivo+"&id_estado="+idestado+"&id_accesorios="+idaccesorios,
					success: function(respuesta){
